...
## すでに ccskill-gmail を使っている方へ

**このセクションは、プロジェクトごとに `ccskill-gmail install` していた方向け**です（プロジェクトディレクトリごとに専用の Gmail（GAS）を持つ、以前の使い方）。現在のバージョンは、アカウントを一度だけ登録すればどのプロジェクトからでも使え、複数アカウントの使い分けにも対応しています。

**特に何もする必要はありません。今までのセットアップはそのまま動き続けます。** 新しい共有方式に切り替えたい、または複数アカウントを使いたい場合のみ、以下をお読みください。

### 移行を選ばない限り何も変わらない

`.ccskill-gmail/` フォルダを持つプロジェクトは、そのフォルダ内のローカル設定で解決し、新しい共有アカウント一覧には一切触れません。既存のプロジェクトは、これまでどおりそのまま使い続けて問題ありません。

### 共有方式への切り替え（任意）

アカウントを一度登録すればどのプロジェクトからでも使いたい、または複数の Gmail アカウントを使い分けたい場合:

```bash
ccskill-gmail account add              # アカウントを一度だけ登録（既存のログインを再利用）
ccskill-gmail migrate --dry-run        # どのプロジェクトがどのアカウントに移るかをプレビュー
ccskill-gmail migrate                  # 既存のプロジェクトインストールを共有アカウントへ移す
ccskill-gmail unbind --purge-legacy    # （任意・プロジェクトごと）古いプロジェクト単位のファイルを削除
```

- `migrate` は**再ログインを求めません**。また**何も削除しません** — 使われなくなったプロジェクト専用のデプロイは*一覧表示するだけ*なので、必要なら [script.google.com](https://script.google.com) から手動で削除できます。カスタムした権限設定（`config.js`）は引き継がれます。
- 古いプロジェクト設定はそのまま残ります。削除されるのは `unbind --purge-legacy` のときだけです（ローカルの履歴ログは保持されます）。

### 注意: 1つのアカウントにデプロイが2つあることも

移行後、あるプロジェクトは**共有**のデプロイを使い始める一方、**古いプロジェクト専用**のデプロイも残っている、という状態になることがあります。同じ Gmail アカウントですが、デプロイは別々の2つです。変更したのに「反映されていない」と感じたら、どちらを使っているか確認してください:

```bash
ccskill-gmail api whoami               # 使用中のアカウントと正確なエンドポイントを表示
```
