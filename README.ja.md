...
検索・閲覧・下書き作成・ラベル整理に加え、添付ファイルのダウンロードやメールの PDF 保存にも対応しています。一括操作（既読/未読、ラベル追加/削除、アーカイブ）にも対応しており、大量メールの効率的な処理が可能です。対応が必要なメールを確認する際は、自動通知を除外して人からのメールだけを抽出し（スマートフィルタ）、内容・状態・対応案を構造化して提示します。
...
**シェルスクリプト生成** — 対話を超えた Gmail 自動化

`.ccskill-gmail/api` コマンドはシェルスクリプトから呼べる Gmail ブリッジ API としても機能します。やりたいことを伝えるだけで、Claude Code が動作するスクリプトを生成します。API の調査は不要です。
