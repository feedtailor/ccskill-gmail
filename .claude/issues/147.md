...
- [x] `lib/accounts.sh` の `label` キーが quote されている
- [x] 回帰テスト（新規）が追加され、修正前は fail・修正後は pass する
- [x] 既存の `tests/lib/test-account-label.sh` / `test-accounts.sh` が pass する
- [x] jq 1.5 相当の厳格パースで構文エラーにならないことを確認する（quote は全 jq 版で有効な構文）
...
- 2026-07-05: fix コミット d034f5f。`fix/147-jq-reserved-label-key` を main へ
