...
プロジェクトごとに異なる Google アカウントを使い分けたい場合は、`--user` オプションを使います。Google ログインが必要な場合はインストーラーが自動で案内します。`cd` するだけでアカウントが自動的に切り替わります。
...
`--user` には英数字・ハイフン・アンダースコアのみ使用できます（例: `work`, `personal`, `info-ft`）。メールアドレスは使用できません。
