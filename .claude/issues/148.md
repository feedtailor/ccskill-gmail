...
- [ ] doctor が jq < 1.6 で WARN を表示し、アップグレード手順を案内する
- [ ] バージョン比較関数の単体テストが追加され pass する
- [ ] jq 1.7.1-apple（開発環境）では警告が出ない（誤検知しない）
- [ ] 不明・空のバージョン文字列で誤警告しない
...
- 2026-07-05: 起票（#147 の再発防止依頼）。TDD で `tests/lib/test-jq-version.sh` を先に追加し
  ...
  関連 lib テスト（jq-version / accounts-jq-reserved / account-label / accounts）すべて pass。（コミット前）
