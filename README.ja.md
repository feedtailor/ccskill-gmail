...
- **プロンプトインジェクション対策** HTML メールに埋め込まれた隠し指示（CSS 非表示・ゼロ幅文字・白文字等）は無効化します
- **Googleアカウント認証前提** Google アカウントで認証することを前提とした作りになっています。

## 使用例
### 複数アカウントを横断して扱う

> 「yamada@ccskill.co.jp と ytaro@gmail.com の両方のアカウントで、今週まだ返信していないメールを探して、アカウント別に送信者と件名を一覧にして」
...
> 途中ブラウザでGoogle認証が求められるので「許可」してください。なお、個人の Google アカウントでは「このアプリは確認されていません」という警告が出ます。個人用 Google アカウントの仕様ですので、[トラブルシューティング](docs/troubleshooting.ja.md)を参照して操作して下さい。
...
認証後、追加したアカウントのラベル(識別子)の入力を求められます。`work` や `personal` など任意の文字列を指定して下さい。指定したラベルで Claude Code に「workのメールを調べて」のように依頼することができます。
